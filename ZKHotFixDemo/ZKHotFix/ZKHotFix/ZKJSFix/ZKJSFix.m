...
@implementation ZKJSFix

+ (void)fixIt {
    JSContext *context = [self context];
    // 修复指定方法
    // fixMethod(类名,方法名,选项:0-方法后/1-替换/2-方法前,是否类方法,匿名函数:实例+方法对象+方法传参数组)
    context[zk_fixMethod] = ^(NSString *instanceName, NSString *selectorName, AspectOptions options, BOOL isClassMethod, JSValue *fixImpl) {
    ...
    context[zk_setInvocationArgs] = ^(NSInvocation *invocation, NSArray *arguments) {
        [self setInvocationArgsWithInvocation:invocation arguments:arguments];
    };
    // 修改该方法返回值
    // setInvocationReturnValue(方法对象,返回值)
    context[zk_setInvocationReturnValue] = ^(NSInvocation *invocation, id returnValue) {
        [self setInvocationReturnValueWithInvocation:invocation value:returnValue];
    };
    // 立刻调用该方法
    // runInvocation(方法对象)
    context[zk_runInvocation] = ^(NSInvocation *invocation) {
        [invocation invoke];
    };
    
    // 调用+方法
    // runClassMethod(类名,方法名,传参数组)
    context[zk_runClassMethod] = ^id(NSString *className, NSString *selectorName, NSArray *arguments) {
        return [self runClassWithClassName:className selector:selectorName arguments:arguments];
    };
    // 调用-方法
    // runClassMethod(实例类名,方法名,传参数组)
    context[zk_runInstanceMethod] = ^id(NSString * className, NSString *selectorName, NSArray *arguments) {
...
+ (void)fixWithMethod:(BOOL)isClassMethod options:(AspectOptions)options
         instanceName:(NSString *)instanceName selectorName:(NSString *)selectorName fixImp:(JSValue *)fixImpl {
